...
serializer = URLSafeTimedSerializer("api-key")
...
def login():
    ...
    try:
        id = db.session.query(User.id).where(and_(User.username == username, User.password == password, User.role == UserRole.Admin.value)).first()
        if not id:
        ...
        response.set_cookie("admin_userid", encrypted_user_id, max_age=60 * 60 * 24 * 3, httponly=True, secure=True, samesite='None')
